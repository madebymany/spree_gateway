module Spree
  class Gateway::SpreedlyGateway < Gateway
    ...
    def create_profile(payment)
      ...
      # Reload the order so we actually get the addresses
      payment.order.reload

      options = {
        email: payment.order.email
      }.merge! addresses_for(payment)
    ...
    def update(identifier, creditcard, options = {})
      identifier = creditcard.gateway_customer_profile_id || identifier
      provider.update(identifier, options)
    end

  private

    def addresses_for(payment)
      { billing_address:  payment.order.bill_address.try(:active_merchant_hash),
